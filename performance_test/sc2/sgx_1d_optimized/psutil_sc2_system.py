...
data = []
header_system = ["flag", "system_memory.percent", "system_memory.total", "system_memory.available", "system_cpu_freq.current", "cpu01", "cpu02", "cpu03", "cpu04", "cpu05", "cpu06", "cpu07", "cpu08", "cpu09", "cpu10", "cpu11", "cpu12"]
...
def monitor_process(pid):
	try:
		...
		cpu_percent = process.cpu_percent(interval=0.1); #cpu_count = 12
...
while True:
	...
	if type(process_pid)!= bool:
		if psutil.pid_exists(process_pid):
			...
			system_cpu_percent = psutil.cpu_percent(interval=0.1, percpu=True)
			system_memory = psutil.virtual_memory();
			row = ["True", system_memory.percent, system_memory.total, system_memory.available, system_cpu_freq.current]
			row.extend(system_cpu_percent)
			data.append(row);
		else:
			system_cpu_freq = psutil.cpu_freq();
			system_cpu_percent = psutil.cpu_percent(interval=0.1, percpu=True)	
			system_memory = psutil.virtual_memory();
			row = ["False", system_memory.percent, system_memory.total, system_memory.available, system_cpu_freq.current]
			row.extend(system_cpu_percent)
			data.append(row);
			break
...
with open('performance_sc2_r.csv', 'w', encoding='UTF8', newline='') as f:
    writer = csv.writer(f)
